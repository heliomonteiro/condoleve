...
const APP_VERSION = "3.2.3-visual-update";
...
const Logo = ({ className = "", dark = false }) => (
  <div className={`flex items-center gap-2 ${className}`}>
    ...
    <div className="flex flex-col leading-none text-left">
        ...
        <span className={`text-[8px] uppercase font-bold tracking-[0.2em] ${dark ? 'text-blue-200' : 'text-slate-400'}`}>Cloud v3.2.3</span>
    </div>
  </div>
);
        ...
        {abaAtiva === 'caixa' && (
          <div className="space-y-6 animate-in fade-in duration-500 text-left">
             <section>
                 ...
                 {/* GRÁFICO CSS CORRIGIDO E AUMENTADO */}
                     ...
                     <Card className="p-6">
                         ...
                         <div className="flex justify-between items-end gap-3 h-40">
                             {historicoGrafico.map((h, i) => {
                                 ...
                                 const hRec = Math.max((h.receita / maxVal) * 100, 2);
                                 const hDesp = Math.max((h.despesa / maxVal) * 100, 2);
                                 return (
                                     <div key={i} className="flex flex-col items-center flex-1 group relative h-full justify-end">
                                         <div className="flex gap-1 items-end w-full justify-center h-full bg-slate-50/50 rounded-lg pb-0 relative">
                                            {/* Receita */}
                                            <div style={{height: `${hRec}%`}} className="w-4 sm:w-6 bg-green-400 rounded-t-sm transition-all group-hover:bg-green-500 relative cursor-pointer shadow-sm">
                                                <div className="absolute bottom-full mb-1 left-1/2 -translate-x-1/2 text-[9px] font-bold text-green-700 bg-green-100 px-1.5 py-0.5 rounded shadow-sm opacity-0 group-hover:opacity-100 transition whitespace-nowrap z-10 pointer-events-none">{formatarMoeda(h.receita)}</div>
                                            </div>
                                            {/* Despesa */}
                                            <div style={{height: `${hDesp}%`}} className="w-4 sm:w-6 bg-red-400 rounded-t-sm transition-all group-hover:bg-red-500 relative cursor-pointer shadow-sm">
                                                <div className="absolute bottom-full mb-1 left-1/2 -translate-x-1/2 text-[9px] font-bold text-red-700 bg-red-100 px-1.5 py-0.5 rounded shadow-sm opacity-0 group-hover:opacity-100 transition whitespace-nowrap z-10 pointer-events-none">{formatarMoeda(h.despesa)}</div>
                                            </div>
                                         </div>
...
function ModalRelatorioCompleto({ mes, ano, receita, gasto, pagamentos, despesas, sindica, unidades, onClose, config }) { 
    ...
    return (
        <div className="fixed inset-0 bg-[#1e293b]/90 z-[100] flex items-center justify-center p-0 md:p-6 print-area backdrop-blur-md text-left text-center">
            <div className="bg-white w-full h-full md:h-auto md:max-h-[90vh] md:max-w-md md:rounded-[32px] shadow-2xl overflow-y-auto flex flex-col relative border-4 border-[#1e293b]">
                {/* Header Compacto */}
                <div className="p-5 border-b flex justify-between items-center bg-slate-50 no-print sticky top-0 z-10 text-left">
                    <h3 className="font-black text-xs uppercase text-slate-400 tracking-widest">Resumo Mensal</h3>
                    <button onClick={onClose} className="p-2 bg-white rounded-full shadow-sm hover:bg-slate-200"><X size={18}/></button>
                </div>

                <div className="p-6 md:p-8 bg-white flex-1 relative print:p-0">
                    {/* Cabeçalho Relatório Estilo Cupom */}
                    <div className="flex justify-between items-center mb-6">
                        <Logo className="scale-75 origin-left" />
                        <div className="text-right">
                            <h1 className="text-lg font-black uppercase tracking-tight text-[#1e293b]">{safeStr(mes)}/{safeStr(ano)}</h1>
                            <p className="font-bold text-slate-400 text-[10px] uppercase tracking-widest">Fechamento</p>
                        </div>
                    </div>

                    {/* Bloco de Resumo Financeiro */}
                    <div className="bg-slate-50 border border-slate-100 rounded-2xl p-4 grid grid-cols-3 divide-x divide-slate-200 mb-6">
                        <div className="text-center px-1">
                            <span className="text-[9px] font-black uppercase text-slate-400 block mb-1">Receita</span>
                            <span className="font-black text-sm text-green-600 block">{formatarMoeda(receita)}</span>
                        </div>
                        <div className="text-center px-1">
                            <span className="text-[9px] font-black uppercase text-slate-400 block mb-1">Despesa</span>
                            <span className="font-black text-sm text-red-500 block">-{formatarMoeda(gasto)}</span>
                        </div>
                        <div className="text-center px-1">
                            <span className="text-[9px] font-black uppercase text-slate-400 block mb-1">Saldo</span>
                            <span className={`font-black text-sm block ${(receita-gasto)>=0?'text-[#1e293b]':'text-red-600'}`}>{formatarMoeda(receita-gasto)}</span>
                        </div>
                    </div>

                    {/* Listas Lado a Lado (Compactas) */}
                    <div className="grid grid-cols-1 gap-6 text-left">
                        {/* Despesas */}
                        <div>
                            <h4 className="font-black text-slate-400 uppercase text-[9px] border-b pb-1 mb-2 tracking-widest flex items-center gap-1"><ArrowDownCircle size={10}/> Saídas</h4>
                            <div className="space-y-1.5">
                                {despesas.length > 0 ? despesas.map(d => (
                                    <div key={d.id} className="flex justify-between text-[11px] items-center">
                                        <span className="font-bold text-slate-600 truncate max-w-[120px]">{safeStr(d.descricao)}</span>
                                        <span className="font-bold text-red-500">-{formatarMoeda(d.valor)}</span>
                                    </div>
                                )) : <p className="text-slate-300 text-[10px] italic">Sem gastos.</p>}
                            </div>
                        </div>

                        {/* Unidades */}
                        <div>
                            <h4 className="font-black text-slate-400 uppercase text-[9px] border-b pb-1 mb-2 tracking-widest flex items-center gap-1"><Home size={10}/> Aptos</h4>
                            <div className="grid grid-cols-2 gap-x-4 gap-y-1.5">
                                {listaUnificada.map(u => (
                                    <div key={u.id} className="flex justify-between text-[11px] items-center">
                                        <span className="font-bold text-slate-700">Apto {safeStr(u.numero)}</span>
                                        {u.status === 'pago' ? <Check size={12} className="text-green-500"/> : <span className="w-2 h-2 rounded-full bg-red-400"></span>}
                                    </div>
                                ))}
                            </div>
                        </div>
                    ...
                    <div className="mt-8 pt-4 border-t border-dashed border-slate-200 text-center">
                        <p className="font-black text-sm text-[#1e293b]">{safeStr(sindica)}</p>
                        <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mt-0.5">Administração</p>
                    </div>
                </div>

                {/* Botões de Ação */}
                <div className="p-4 bg-slate-50 border-t flex gap-2 no-print sticky bottom-0 rounded-b-[30px]">
                    <button onClick={gerarResumoZap} className="bg-green-600 text-white py-3 rounded-xl font-black text-xs shadow-md flex items-center justify-center gap-2 hover:bg-green-700 transition flex-1"><MessageCircle size={16}/> WHATSAPP</button>
                    <button onClick={() => window.print()} className="bg-[#1e293b] text-white py-3 rounded-xl font-black text-xs flex items-center justify-center gap-2 shadow-md hover:bg-black transition flex-1"><Printer size={16}/> IMPRIMIR / PDF</button>
                </div>
            </div>
        </div>
    );
